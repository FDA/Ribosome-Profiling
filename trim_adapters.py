...
def trim_adapters(dataset, base_name):
    ...
    p2 = subprocess.Popen(fastx_cmd, shell = True, stdin = p1.stdout, stderr = log_fh)  # it's stderr because fastx_clipper is dumb, see its -h info
    p2.wait()
    ...
    gzip_cmd = "gzip ./Trimmed/%s/%s.fastq" % (dataset, base_name)
